...
#include "Device/Graphics/String/TextureChar.h"
#include "Device/Graphics/Sprite/Sprite2D.h"
#include "Utility/Math/Vector2.h"

namespace Graphics {

TextureString::TextureString(GraphicsDeviceManager& graphicsDevice,
...
void TextureString::create() {
    ...
    mTextureChars.clear();
    //まずは一文字目を作成
    std::unique_ptr<TextureChar> p = std::make_unique<TextureChar>(
        mGraphicsDevice, mStr.substr(0, 1).c_str(), mFontSize, mFont);
    p->setPosition(mPosition);
    mTextureChars.emplace_back(std::move(p));
    //2文字目からは前の文字の座標からずらした場所に生成
    for (int i = 1; i < size; i++) {
        p = std::make_unique<TextureChar>(mGraphicsDevice, mStr.substr(i, 1).c_str(), mFontSize, mFont);
