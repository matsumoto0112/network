...
TextureChar::TextureChar(GraphicsDeviceManager& graphicsDevice, const char* c, int fontSize, const std::string& font)
    :Sprite2D(graphicsDevice) {
    LOGFONT lf = {
        fontSize, //高さ
        0, // 幅
        0, //文字送りの方向とX軸との角度
    ...
    std::shared_ptr<TextureBuffer> tex = std::make_shared<TextureBuffer>(
        graphicsDevice.getDirectX11Device(), desc);

    D3D11_MAPPED_SUBRESOURCE hMappedResource;
    HRESULT hr = graphicsDevice.getContext()->Map(
        tex->getBuffer().Get(),
        0,
    ...
    graphicsDevice.getContext()->Unmap(tex->getBuffer().Get(), 0);
    ...
    std::shared_ptr<ShaderResourceView> srv =
        std::make_unique<ShaderResourceView>(mGraphicsDevice.getDirectX11Device(), *tex, srvDesc);

    mTexture = std::make_shared<Texture>(graphicsDevice, tex, srv, GM.gmCellIncX, TM.tmHeight);
