...
Sprite2D::Sprite2D(GraphicsDeviceManager& graphicsDevice)
...
Sprite2D::Sprite2D(GraphicsDeviceManager& graphicsDevice, std::shared_ptr<Texture>& texture)
    :mGraphicsDevice(graphicsDevice),
    mTexture(texture),
    mPosition(0, 0),
    mZOrder(0.0f),
...
void Sprite2D::draw() {
    mGraphicsDevice.getSpriteRenderer().draw(this);
}
