...
float lengthSegmentOnSearateAxis(
    const Math::Vector3& sep,
    const Math::Vector3& e1,
    const Math::Vector3& e2,
    const Math::Vector3& e3 = Math::Vector3::ZERO) {
    float r1 = Math::MathUtility::abs(sep.dot(e1));
    float r2 = Math::MathUtility::abs(sep.dot(e2));
    float r3 = e3 != Math::Vector3::ZERO ? Math::MathUtility::abs(sep.dot(e3)) : 0;
    return r1 + r2 + r3;
}
}
