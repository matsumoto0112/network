...
namespace {
static const Math::Vector3 DefaultNormal(0.0f, 0.0f, 1.0f);
static const Math::Vector2 DefaultUV(0.0f, 0.0f);
static const Graphics::Color4 DefaultColor(1.0f, 1.0f, 1.0f, 1.0f);

Graphics::Color4 convertFromFbxDouble(const FbxDouble3& value, double mult) {
    Graphics::Color4 result;
    if (mult != 1) {
...
Math::Vector4 convertPositionFromSourcePosition(const FbxVector4& pos) {
    Math::Vector4 res;
    //xだけ反転しているっぽいので修正
    res.x = -(float)pos[0];
    res.y = (float)pos[1];
...
Math::Vector3 convertNormalFromSourceNormal(const FbxVector4& normal) {
    Math::Vector3 res;
...
Math::Vector2 convertUVFromSourceUV(const FbxVector2& uv) {
    Math::Vector2 res;
    res.x = (float)uv[0];
...
Graphics::Color4 convertColorFromSourceColor(const FbxColor& color) {
    Graphics::Color4 res;
...
FBXLoader::FBXLoader(GraphicsDeviceManager& grpahicsDevice)
    :mGraphicsDevice(grpahicsDevice),
    mTexLoader(std::make_unique<TextureLoader>(grpahicsDevice)) {
...
std::unique_ptr<Model> FBXLoader::load(const std::string& filepath) {
    ...
    return std::make_unique<Model>(mGraphicsDevice, std::move(meshes), std::move(materials));
}
...
std::unique_ptr<AnimationModel> FBXLoader::loadWithAnimation(const std::string& filepath) {
    ...
    return std::make_unique<AnimationModel>(mGraphicsDevice,
        std::move(meshes), std::move(materials), std::move(pBones), std::move(animation));
}
...
std::unique_ptr<Mesh> FBXLoader::loadMesh(FbxMesh* mesh) {
    ...
    result->mVIBuffer = std::make_unique<VertexAndIndexBuffer>(mGraphicsDevice, vertices, indices,
    ...
    if (mapMode == FbxLayerElement::eByPolygonVertex) {
        if (refMode == FbxLayerElement::eIndexToDirect) {
            ...
            for (int i = 0; i < indices.GetCount(); i++) {
                ...
                FbxColor& color = colors.GetAt(indices.GetAt(i));
...
LoadEnd:
    result->mVIBuffer = std::make_unique<VertexAndIndexBuffer>(mGraphicsDevice, vertices, indices,
        PrimitiveTopology::TriangleList);
    result->mVShader = std::make_unique<VertexShader>(mGraphicsDevice, "AnimationVS");
    result->mPShader = std::make_unique<PixelShader>(mGraphicsDevice, "AnimationPS");
