...
TextureLoader::TextureLoader(GraphicsDeviceManager& graphicsDevice)
...
std::unique_ptr<Texture> TextureLoader::load(const std::string& filepath) {
    ...
    //テクスチャ作成
    std::shared_ptr<TextureBuffer> texture2D = std::make_shared<TextureBuffer>(
        mGraphicsDevice.getDirectX11Device(), mTexture2DDesc, sub);
    std::shared_ptr<ShaderResourceView> srv =
        std::make_shared<ShaderResourceView>(mGraphicsDevice.getDirectX11Device(), *texture2D, mSRVDesc);

    return std::make_unique<Texture>(mGraphicsDevice, texture2D, srv, width, height);
}
} //Graphics 
