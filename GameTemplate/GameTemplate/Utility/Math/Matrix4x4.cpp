...
#include "Utility/Math/Matrix3x3.h"
...
Matrix4x4 Matrix4x4::createView(const Vector3& eye, const Vector3& at, const Vector3& up) {
    Vector3& zaxis = (at - eye).getNormal();
    Vector3& xaxis = up.cross(zaxis).getNormal();
...
Matrix4x4 Matrix4x4::createProjection(float fovY, float screenX, float screenY, float near, float far) {
    float yScale = MathUtility::cot(fovY / 2.0f);
    float xScale = screenY * yScale / screenX;
    float subZ = far - near;
    return Matrix4x4
    (
        xScale, 0, 0, 0,
